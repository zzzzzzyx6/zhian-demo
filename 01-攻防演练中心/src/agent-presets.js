...
for(const [id,name,provider,endpoint] of [['agent-dify','Dify 智能体','dify','https://api.dify.ai/v1/chat-messages'],['agent-coze','Coze 智能体','coze','https://api.coze.cn/v3/chat']])if(!C.db.some(m=>m.id===id))C.db.push({id,name,provider,protocol:provider,kind:'agent',accessType:'api',model:'',endpoint,botId:'',userId:'security-demo',schema:2,sample:false,presetPending:true,auth:'bearer',caps:['text','chat'],uses:['target'],timeout:60});X.db.agentPresetsVersion=1;};
...
const detail=X.modelDetail;X.modelDetail=function(id){const m=X.model(id);if(!['dify','coze'].includes(m.provider))return detail(id);A.drawer('连接配置',`<h3>${E(m.name)}</h3>`+[['平台',m.provider==='dify'?'Dify':'Coze（扣子）'],['服务地址',m.endpoint],...(m.provider==='coze'?[['Bot ID',m.botId||'未填写']]:[]),['调用用户标识',m.userId],['状态',C.status(m)]].map(([k,v])=>`<div class="kv"><span>${k}</span><span style="overflow-wrap:anywhere;min-width:0">${E(v)}</span></div>`).join(''),'');};
})();
